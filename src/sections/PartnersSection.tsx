...
function MarqueeRow({ items, reverse = false }: { items: typeof partners; reverse?: boolean }) {
  ...
  return (
    <div className="overflow-hidden py-4">
      <div className={`flex gap-16 items-center ${reverse ? 'animate-marquee-reverse' : 'animate-marquee'}`} style={{ width: 'max-content' }}>
        {doubled.map((partner, i) => (
          <div key={`${partner.slug}-${i}`} className="group flex items-center gap-3 flex-shrink-0 cursor-pointer" data-cursor-hover>
            <img
  ...
  return (
    <section id="partners" className="py-24 bg-dark border-y border-dark-border relative overflow-hidden">
      ...
      <div className="text-center mb-12 px-6">
        <p className="text-white/30 text-sm tracking-widest uppercase font-medium">{t.partners.heading}</p>
      </div>
