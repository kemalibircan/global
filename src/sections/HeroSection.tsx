...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        colors[i * 3 + 1] = t * 0.83 + (1 - t) * 0.18
        colors[i * 3 + 2] = t * 1 + (1 - t) * 0.75
      }
      ...
      material = new THREE.PointsMaterial({
        size: 0.02, vertexColors: true, transparent: true, opacity: 0.8, sizeAttenuation: true,
      })
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setWordIndex((i) => (i + 1) % h.rotatingWords.length)
    }, 2500)
    return () => clearInterval(interval)
  }, [h.rotatingWords.length])
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-dark" id="hero">
      ...
      <div className="relative z-10 text-center max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <span className="text-neon-blue text-xs font-medium tracking-widest uppercase">{h.badge}</span>
        </motion.div>
        ...
        >
          {h.headline1}{' '}
          <span className="gradient-text text-glow-blue">{h.headline2}</span>
        </motion.h1>
        ...
        >
          <a
            href="#portfolio"
            className="group relative px-8 py-4 rounded-full text-dark font-semibold text-sm overflow-hidden"
            style={{ background: 'linear-gradient(135deg, #00D4FF, #7B2FBE)', transition: 'transform 0.5s cubic-bezier(0.23,1,0.32,1)' }}
            onMouseMove={handleMagneticMove} onMouseLeave={handleMagneticLeave} data-cursor-hover
          >
      ...
      >
        <span className="text-white/30 text-xs tracking-widest uppercase">{h.scrollLabel}</span>
